...
class InventoryDatabase:
    ...
    def init_database(self):
        ...
        # Migrate existing databases to add barcode column
        try:
        ...
        cursor.execute(
            "INSERT OR IGNORE INTO listing_counter (id, next_listing_number) VALUES (1, 1)"
        )
    ...
    def add_item(self, title: str, price: float, quantity: int, condition: str = "",
                 ...
                 barcode: str = "", status: str = "active") -> int:
        ...
        cursor.execute("""
            INSERT INTO inventory
                (title, price, quantity, condition, description, category, image_path, barcode, status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (title, price, quantity, condition, description, category, image_path, barcode, status))
    ...
    def update_item(self, item_id: int, title: str, price: float, quantity: int,
                    ...
                    image_path: str = "", barcode: str = "", status: str = "active") -> bool:
        ...
        cursor.execute("""
            ...
            SET title = ?, price = ?, quantity = ?, condition = ?,
                description = ?, category = ?, image_path = ?, barcode = ?, status = ?,
                updated_at = CURRENT_TIMESTAMP
            WHERE id = ?
        """, (title, price, quantity, condition, description, category,
              image_path, barcode, status, item_id))
    ...
    def search_items(self, search_term: str) -> List[Dict]:
        """Search inventory items by title, description, category, or barcode."""
        conn = self.get_connection()
        cursor = conn.cursor()
        ...
        cursor.execute("""
            ...
            WHERE title LIKE ? OR description LIKE ? OR category LIKE ? OR barcode LIKE ?
    ...
    def search_items_by_status(self, search_term: str, status: str) -> List[Dict]:
        ...
        cursor.execute("""
            ...
            AND status = ?
            ORDER BY created_at DESC
        """, (search_pattern, search_pattern, search_pattern, search_pattern, status))
    ...
    def get_item_by_barcode(self, barcode: str) -> Optional[Dict]:
        ...
        cursor.execute("SELECT * FROM inventory WHERE barcode = ? LIMIT 1", (barcode,))
        row = cursor.fetchone()
    ...
    def get_next_listing_number(self) -> int:
        ...
        cursor.execute(
            "UPDATE listing_counter SET next_listing_number = ? WHERE id = 1",
            (next_num + 1,)
        )
